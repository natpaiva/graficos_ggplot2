...
#Vamos tentar visualizar a relação entre tamanho do motor (displ) e 
# eficiência do carro (hwy)
...
# relacao entre displ e hwy apenas para fabricante audi
...
# grafico de dispersao entre displ e hwy por fabricante
ggplot(data = mpg) +
  aes(x = displ, y = hwy) +
  geom_point() +
...
ggplot(data = mpg) +
  ...
  labs( x = "Engine displacement (litres)",
        y = "Highway miles per gallon (mpg)",
        colour = "Legenda:") # muda de acordo com o argumento que usei no aes
...
#ordenando as colunas com fct_infreq 
# (por frequencia; pacote forcats)
ggplot(data = mpg) +
...
# Tração segundo ano de fabricação
# inserindo frequencia nas colunas
ggplot(data = mpg) +
  aes (x = drv, fill = factor(year)) +
  geom_bar(position=position_dodge()) +
...
# Tração segundo ano de fabricação
# inserindo frequencia e % nas colunas

library(scales) #opa, aqui tem que instalar antes
...
mpg <- mpg %>% mutate( drv2 = factor(drv,
                                     levels = c("f", "r", "4")))
...
ggplot(data = mpg) +
  aes (x = drv, fill = factor(year)) +
  geom_bar(position=position_dodge()) +
  facet_wrap(. ~class, scales = "free") # cuidado!
